...
CREATE TABLE IF NOT EXISTS `SchemaShovelWebAPI`.`table` (
  ...
  INDEX `table_database_idx` (`databaseID` ASC) VISIBLE,
  CONSTRAINT `table_schema`
    FOREIGN KEY (`schemaID`)
    REFERENCES `SchemaShovelWebAPI`.`schema` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `table_database`
...
CREATE TABLE IF NOT EXISTS `SchemaShovelWebAPI`.`column` (
  ...
  `foreignKeyToTableID` INT NULL,
  `description` VARCHAR(2000) NULL,
  PRIMARY KEY (`id`),
  INDEX `column_table_idx` (`tableID` ASC) VISIBLE,
  INDEX `fkColumn_table_idx` (`foreignKeyToTableID` ASC) VISIBLE,
  CONSTRAINT `column_table`
    FOREIGN KEY (`tableID`)
    REFERENCES `SchemaShovelWebAPI`.`table` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fkColumn_table`
    FOREIGN KEY (`foreignKeyToTableID`)
    REFERENCES `SchemaShovelWebAPI`.`table` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
